...
(async () => {
  ...
  await fs.writeFile("type-1-diabetese-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("type-1-diabetese-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
